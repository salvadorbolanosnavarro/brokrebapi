...
async def get_colonias(q: str = Query("", min_length=2), ciudad: str = "Morelia"):
    """Return unique colonias matching search query from EB properties."""
    if not EB_API_KEY:
        raise HTTPException(status_code=500, detail="EB_API_KEY no configurada")

    cache_key = f"colonias_{ciudad.lower()}"
    colonias_map = cache_get(cache_key)

    if colonias_map is None:
        # Build colonia index from EB properties
        all_props = await fetch_all_properties()
        colonias_map = {}  # colonia -> count
        ...
        for p in all_props:
            ...
            if ciudad.lower() not in loc.lower():
                continue
            colonia = extract_colonia(loc)
            if colonia and len(colonia) > 2:
                colonias_map[colonia] = colonias_map.get(colonia, 0) + 1
    ...
    # Sort by count (more properties = more relevant)
    matches.sort(key=lambda x: -x["count"])
    return {"colonias": matches[:10], "total_colonias": len(colonias_map)}
...
async def calcular_avm(req: AVMRequest):
    ...
    return {
        ...
        "nota": ("Valores calculados con base en propiedades reales de la bolsa "
                 "EasyBroker con ajustes hedónicos por m², recámaras, estado y antigüedad. "
